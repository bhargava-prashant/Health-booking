...
// Icons
import { Calendar, Clock, CheckCircle, XCircle, User, Filter, LogOut, Phone, Mail, Info } from 'lucide-react';
...
const DoctorDashboard = () => {
  ...
  const API_USERS_URL = 'http://localhost:5003/api/admin/users';
  const API_DOCTOR_URL = 'http://localhost:5003/api/admin/user';
  const API_PATIENT_URL='http://localhost:5003/api/admin/users';
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (doctorResponse.data) {
        ...
        // Get all users to find patient names and details
        ...
        // Create a map of patient details by user_id for quick lookup
        const patientsMap = {};
        usersResponse.data.forEach(user => {
          if (user.role === 'patient') {
            patientsMap[user.user_id] = user;
          }
        });
        ...
        const bookingsData = bookingsResponse.data;
        const mappedBookings = bookingsData.map(booking => {
          const patient = patientsMap[booking.patient_id] || null;

          return {
            ...booking,
            id: booking.booking_id || `${booking.doctor_id}-${booking.patient_id}-${booking.date}-${booking.time}`,
            patient_name: patient ? `${patient.first_name} ${patient.last_name}` : 'Unknown Patient',
            patient: patient, // Store the full patient object
            formattedDate: new Date(booking.date).toLocaleDateString('en-US', {
              weekday: 'long',
              year: 'numeric',
              month: 'long',
  ...
  const togglePatientDetails = (bookingId) => {
            ...
            <div key={booking.id} style={styles.bookingCard}>
              <div style={styles.bookingHeader}>
                <div style={styles.patientBasicInfo}>
                  <h3 style={styles.patientName}>{booking.patient_name}</h3>
                  <p style={styles.patientId}>Patient ID: {booking.patient_id}</p>
                </div>
                <button 
                  onClick={() => togglePatientDetails(booking.id)}
              ...
              <p style={styles.appointmentTime}>
                ...
                {booking.formattedDate} at {booking.time}
              </p>
              ...
              <p style={styles.statusLabel}>
                ...
                Status: <span style={{
                  ...styles.statusBadge,
                  backgroundColor: 
                    booking.status === 'scheduled' ? '#3498db' : 
              ...
              {expandedBooking === booking.id && booking.patient && (
                <div style={styles.expandedDetails}>
                  <h4 style={styles.expandedTitle}>Patient Details</h4>
                  <div style={styles.detailsGrid}>
                    <div style={styles.detailItem}>
                      <User size={16} style={{ marginRight: '8px' }} />
                      <p>Full Name: {booking.patient.first_name} {booking.patient.last_name}</p>
                    </div>
                    <div style={styles.detailItem}>
                      <Mail size={16} style={{ marginRight: '8px' }} />
                      <p>Email: {booking.patient.email || 'Not provided'}</p>
                    </div>
                    <div style={styles.detailItem}>
                      <Phone size={16} style={{ marginRight: '8px' }} />
                      <p>Phone: {booking.patient.phone || 'Not provided'}</p>
                    </div>
                    {booking.patient.address && (
                      <div style={styles.detailItem}>
                        <Info size={16} style={{ marginRight: '8px' }} />
                        <p>Address: {booking.patient.address}</p>
                      </div>
                    )}
                    {booking.patient.gender && (
                      <div style={styles.detailItem}>
                        <Info size={16} style={{ marginRight: '8px' }} />
                        <p>Gender: {booking.patient.gender}</p>
                      </div>
                    )}
                    {booking.patient.date_of_birth && (
                      <div style={styles.detailItem}>
                        <Calendar size={16} style={{ marginRight: '8px' }} />
                        <p>Date of Birth: {new Date(booking.patient.date_of_birth).toLocaleDateString()}</p>
                      </div>
                    )}
                    {booking.notes && (
                      <div style={styles.detailItem}>
              ...
              <div style={styles.actionButtons}>
                {booking.status === 'scheduled' && (
                  <>
                    ...
                    >
                      <CheckCircle size={16} style={{ marginRight: '5px' }} /> Complete
                    </button>
                    <button
                      onClick={() => handleStatusChange(booking, 'cancelled')}
